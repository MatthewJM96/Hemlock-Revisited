...
struct TVS_VoxelGenerator {
    void operator()(hmem::Handle<hvox::Chunk> chunk) const {
            ...
            for (ui8 z = 0; z < CHUNK_LENGTH; ++z) {
                for (ui8 y = 0; y < CHUNK_LENGTH; ++y) {
                    for (ui8 x = 0; x < CHUNK_LENGTH; ++x) {
                        chunk->blocks[hvox::block_index({ x, CHUNK_LENGTH - y - 1, z }
        ...
        // hvox::BlockChunkPosition{CHUNK_LENGTH - 1}, hvox::Block{0}); ui64 noise_idx
        // = 0; for (ui8 z = 0; z < CHUNK_LENGTH; ++z) {
        //     for (ui8 x = 0; x < CHUNK_LENGTH; ++x) {
        ...
        //         hvox::set_blocks(chunk, {x, 0, z}, {x, glm::min(height - y,
        //         CHUNK_LENGTH - 1), z}, hvox::Block{1});
        //     }
        // }
...
public:
    VoxelPhysDrawer(hcam::BasicFirstPersonCamera* camera, hg::GLSLProgram* shader) :
        btIDebugDraw() {
        m_camera      = camera;
        ...
        glNamedBufferData(m_vbo, sizeof(f32v3) * 2, nullptr, GL_DYNAMIC_DRAW);
    ...
    virtual void
    drawLine(const btVector3& from, const btVector3& to, const btVector3&) override {
        f32v3 points[2] = {
            f32v3{from.x() /* + m_camera->position().x*/,
                  from.y() /* + m_camera->position().y*/,
                  from.z() /* + m_camera->position().z*/},
            f32v3{  to.x() /* + m_camera->position().x*/,
        ...
        glUniformMatrix4fv(
            m_line_shader->uniform_location("view_proj"),
            1,
            GL_FALSE,
    ...
    virtual void
    drawContactPoint(const btVector3&, const btVector3&, btScalar, int, const btVector3&)
        override { /* Empty. */
    }

    virtual void reportErrorWarning(const char* warning_string [[maybe_unused]]
    ) override {
...
public:
    ...
    virtual void update(hemlock::FrameTime time) override {
        ...
        m_player.ac.position += hvox::EntityWorldPosition{
            f32v3{delta_pos.x, delta_pos.y, delta_pos.z}
            * static_cast<f32>(1ll << 32)
        };
                 ...
                 ++z)
            {
                for (auto y = -2; y < 6; ++y) {
                    ...
                    m_chunk_grid->unload_chunk_at(
                        {
                            {x_step < 0 ?
static_cast<i32>(current_pos.x) + VIEW_DIST + 1 :
static_cast<i32>(current_pos.x) - VIEW_DIST - 1,
                             y, z}
                    },
                        &handle
                 ...
                 ++z)
            {
                for (auto y = -2; y < 6; ++y) {
                    m_chunk_grid->preload_chunk_at({
                        {x_step < 0 ? static_cast<i32>(current_pos.x) - VIEW_DIST :
...
static_cast<i32>(current_pos.x) + VIEW_DIST,
                    ...
                    m_chunk_grid->unload_chunk_at(
                        {
                            {x,
                             y, z_step < 0 ?
 static_cast<i32>(current_pos.z) + VIEW_DIST + 1 :
 static_cast<i32>(current_pos.z) - VIEW_DIST - 1}
                    },
                        &handle
                    );
                 ...
                 ++x)
            {
                for (auto y = -2; y < 6; ++y) {
                    m_chunk_grid->preload_chunk_at({
                        {x,
                         y, z_step < 0 ? static_cast<i32>(current_pos.z) - VIEW_DIST :
 ...
 static_cast<i32>(current_pos.z) + VIEW_DIST}
        ...
        if (hphys::ChunkGridCollider::determine_candidate_colliding_voxels<
                TVS_VoxelShapeEvaluator>(
                m_player.ac, m_player.dc, m_player.cc, voxel_patch
            ))
        {
            btTransform transform = btTransform::getIdentity();
            transform.setOrigin(btVector3(
                glm::floor(m_camera.position().x),
                glm::floor(m_camera.position().y),
                glm::floor(m_camera.position().z)
            ));
            ...
            btRigidBody::btRigidBodyConstructionInfo body_info
                = btRigidBody::btRigidBodyConstructionInfo(
                    mass, motion_state, voxel_patch, inertia
                );
        ...
        m_phys.world->stepSimulation(
            hemlock::frame_time_to_floating<std::chrono::seconds, btScalar>(time)
        );
        ...
        m_player.ac.position = hvox::EntityWorldPosition{
            static_cast<hvox::EntityWorldPositionCoord>(
                m_camera.position().x * static_cast<f32>(1ll << 32)
            ),
            static_cast<hvox::EntityWorldPositionCoord>(
                m_camera.position().y * static_cast<f32>(1ll << 32)
            ),
            static_cast<hvox::EntityWorldPositionCoord>(
                m_camera.position().z * static_cast<f32>(1ll << 32)
            )
        };
    ...
    virtual void init(const std::string& name, happ::ProcessBase* process) override {
        ...
        m_player.cc.shape->addChildShape(
            btTransform::getIdentity(), new btBoxShape(btVector3{ 0.5f, 1.5f, 0.5f })
        );
        ...
        m_phys.world            = new btDiscreteDynamicsWorld(
            m_phys.dispatcher,
            m_phys.broadphase,
            m_phys.solver,
            m_phys.collision_config
        );
...

        {
            ...
            btRigidBody::btRigidBodyConstructionInfo body_info
                = btRigidBody::btRigidBodyConstructionInfo(
                    mass, motion_state, m_player.cc.shape, inertia
                );
            body_info.m_restitution = 0.0f;
