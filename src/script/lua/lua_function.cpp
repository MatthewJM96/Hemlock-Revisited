...
hscript::ScriptDelegate<hscript::CallParameters, hscript::CallParameters> hscript::lua::
    make_arbitrary_scalars_lua_delegate(LuaFunctionState lua_func_state, i32 expected_return_count)
{
    using ReturnType = decltype(ScriptDelegate<CallParameters, CallParameters>()(CallParameters{}));
    ...
    return ScriptDelegate<CallParameters, CallParameters>{
        [lua_func_state, expected_return_count](CallParameters parameters) {
            ...
            i32 return_count = expected_return_count < H_MAX_ARBITRARY_LUA_RETURNS ? expected_return_count : H_MAX_ARBITRARY_LUA_RETURNS;
            ...
            // We sdon't want to fail on unrecognised return type. Caller has control only
            // of their environment, so they can pass Nil for unsupported parameters,
            // but if a function they need to call returns something we can't pass over
            // they simply cannot call it if we fail on this..
