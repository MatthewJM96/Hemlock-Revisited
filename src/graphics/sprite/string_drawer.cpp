...
void hg::add_string_no_wrap(
    SpriteBatcher*          batcher,
    DrawableStringComponent component,
    f32v4                   target_rect,
    f32v4                   clip_rect,
    TextAlign               align,
    f32                     depth
...
void hg::add_string_no_wrap(
    SpriteBatcher*           batcher,
    DrawableStringComponents components,
    ui32                     num_components,
    f32v4                    target_rect,
    f32v4                    clip_rect,
    TextAlign                align,
    ...
    // TODO(Matthew): Can we make guesses as to the amount of drawables to reserve for
    // a line? For amount of lines?
    ...
    for (ui32 component_idx = 0; component_idx < num_components; ++component_idx) {
        ...
        for (size_t i = 0; str[i] != '\0'; ++i) {
            ...
            size_t character_index
                = static_cast<size_t>(character) - static_cast<size_t>(start);

            // If character is a new line character, add a new line and go to next
            // character.
            if (character == '\n') {
                total_height += lines.back().height;
                lines.emplace_back(DrawableLine{
                    0.0f, height, std::vector<DrawableGlyph>() });
            ...
            if (character < start || character > end
                || !font.glyphs[character_index].supported)
                continue;

            // If the line's height is less than the height of this font instance, and
            // we're about to add a glyph from this font instance, then the line's
            // height needs setting to the font instances's.
            if (lines.back().height < height) {
                // If we have overflowed the rectangle with the new line height, break
                // out instead.
            ...
            lines.back().drawables.emplace_back(DrawableGlyph{
                &font.glyphs[character_index],
                lines.back().length,
                scaling,
                tint,
                font.texture });
    ...
    for (auto& line : lines) {
        f32v2 offsets
            = calculate_align_offset(align, target_rect, total_height, line.length);

        for (auto& drawable : line.drawables) {
            f32v2 size     = drawable.glyph->size * drawable.scaling;
            f32v2 position = f32v2(drawable.x_pos, current_y) + offsets
                             + f32v2(target_rect.x, target_rect.y)
                             + f32v2(0.0f, line.height - size.y);
...
void hg::add_string_quick_wrap(
    ...
    for (ui32 component_idx = 0; component_idx < num_components; ++component_idx) {
        ...
        // auto hyphenate = [&]() {
        //     char   hyphen = '-';
        //     size_t index  = static_cast<size_t>(hyphen) -
        //     static_cast<size_t>(start);

        //     f32 character_width = font.glyphs[index].size.x * scaling.x;

        //     lines.back().drawables.emplace_back(DrawableGlyph{ &font.glyphs[index],
        //     lines.back().length, scaling, tint, font.texture });
        //     lines.back().length += character_width;
        // };
        ...
        for (ui32 i = 0; str[i] != '\0'; ++i) {
            char character = str[i];
            ui32 character_index
                = static_cast<ui32>(character) - static_cast<ui32>(start);
            ...
            // Given we are about to add a character, make sure it fits on the line,
            // if not, make a new line and if the about-to-be-added character isn't a
            // whitespace revisit it.
...
void hg::add_string_greedy_wrap(
    SpriteBatcher*          batcher,
    DrawableStringComponent component,
    f32v4                   target_rect,
    f32v4                   clip_rect,
...
void hg::add_string_greedy_wrap(
    ...
    for (ui32 i = 0; i < num_components; ++i) {
        ...
        // We have to do per-word lookahead before adding any characters to the lines,
        // these data points enable us to do this.
        ui32 begin_index   = 0;
...
// void hg::add_string_minimum_raggedness_wrap(SpriteBatcher* batcher,
// DrawableStringComponent component, f32v4 target_rect, f32v4 clip_rect, TextAlign
// align, f32 depth) {
//     add_string_minimum_raggedness_wrap(batcher, &component, 1, target_rect,
//     clip_rect, align, depth);
// }

// void hg::add_string_minimum_raggedness_wrap(SpriteBatcher* batcher,
// DrawableStringComponents components, ui32 num_components, f32v4 target_rect, f32v4
// clip_rect, TextAlign align, f32 depth) {
//     std::vector<std::vector<Word>> words;
//     words.resize(components.size());

//     // Split the string of each component into their costituent words in
//     preparation for calculating the appropriate
...
//             char   character      = str[current_index];
//             size_t character_index = static_cast<size_t>(character) -
//             static_cast<size_t>(start);
...
//             //// For any of the breakable characters, add the current word to the
//             list of words
...
//                 continue;
//             // If we are dealing with a hyphen, mark the next word as being
//             hyphenated. } else if (character == '-') {
//                 words[i].push_back(latest);
...
//                 static_cast<size_t>(start); latest.length +=
//                 font.glyphs[index].size.x * scaling.x;
...
//             ++latest.end;
//             // Determine character width after scaling and increment word length by
//             it. latest.length += font.glyphs[character_index].size.x * scaling.x;
//         }
...
//     // TODO(Matthew): Can we make guesses as to the amount of drawables to reserve
//     for a line? For amount of lines?
...
//         // We have to do per-word lookahead before adding any characters to the
//         lines, these data points
...
//                 // Add character to line for drawing.
//                 lines.back().drawables.emplace_back(DrawableGlyph{
//                 &font.glyphs[character_index], lines.back().length, scaling, tint,
//                 font.texture });

//                 // Determine character width after scaling & add to line length.
//                 f32 character_width = font.glyphs[character_index].size.x *
//                 scaling.x; lines.back().length += character_width;
...
//                 total_height += lines.back().height;
//                 lines.emplace_back(DrawableLine{ 0.0f, height,
//                 std::vector<DrawableGlyph>() });
...
//             // For characters on which we may break a line, flush the word so far
//             // prematurely so that the breakable character can be handled
//             correctly. if (character == ' ' || character == '-') {
//                 flush_word_to_line();
//             }

//             // Given we are about to add a character, make sure it fits on the
//             line, if not, make
//             // a new line and if the about-to-be-added character isn't a whitespace
//             revisit it. if (lines.back().length + wordLength + character_width >
//             target_rect.z) {
//                 total_height += lines.back().height;
//                 lines.emplace_back(DrawableLine{ 0.0f, height,
//                 std::vector<DrawableGlyph>() });
...
//             // If the line's height is less than the height of this font instance,
//             and we're about to add a
//             // glyph from this font instance, then the line's height needs setting
//             to the font instances's. if (lines.back().height < height) {
//                 // If we have overflowed the rectangle with the new line height,
//                 break out instead. if (total_height + height > target_rect.w) {
//                     vertical_overflow = true;
//                     break;
//                 }
//                 lines.back().height = height;
...
//     f32 current_y = 0.0f;
//     for (auto& line : lines) {
//         f32v2 offsets = calculate_align_offset(align, target_rect, total_height,
//         line.length);

//         for (auto& drawable : line.drawables) {
//             f32v2 size         = drawable.glyph->size * drawable.scaling;
//             f32v2 position     = f32v2(drawable.x_pos, current_y) + offsets +
//             f32v2(target_rect.x, target_rect.y) + f32v2(0.0f, line.height -
//             size.y); f32v4 uv_rect = drawable.glyph->uv_rect;

//             clip(clip_rect, position, size, uv_rect);

//             batcher->add_sprite(drawable.texture, position, size, drawable.tint,
//                                      { 255, 255, 255, 255 }, Gradient::NONE, depth,
//                                      uv_rect);
//         }
