...
hvox::block_chunk_position(BlockWorldPosition block_world_position) {
    return { static_cast<BlockChunkPositionCoord>(
                 (block_world_position.x % CHUNK_LENGTH + CHUNK_LENGTH) % CHUNK_LENGTH
             ),
             static_cast<BlockChunkPositionCoord>(
                 (block_world_position.y % CHUNK_LENGTH + CHUNK_LENGTH) % CHUNK_LENGTH
             ),
             static_cast<BlockChunkPositionCoord>(
                 (block_world_position.z % CHUNK_LENGTH + CHUNK_LENGTH) % CHUNK_LENGTH
             ) };
}

hvox::BlockChunkPosition hvox::block_chunk_position(ui32 index) {
    BlockChunkPositionCoord z = static_cast<BlockChunkPositionCoord>(
        static_cast<f32>(index) / static_cast<f32>(CHUNK_AREA)
    );

    index %= CHUNK_AREA;

    BlockChunkPositionCoord y = static_cast<BlockChunkPositionCoord>(
        static_cast<f32>(index) / static_cast<f32>(CHUNK_LENGTH)
    );

    BlockChunkPositionCoord x
        = static_cast<BlockChunkPositionCoord>(index % CHUNK_LENGTH);

    return { x, y, z };
}

hvox::BlockWorldPosition hvox::block_world_position(f32v3 position) {
    return { static_cast<BlockWorldPositionCoord>(glm::floor(position.x)),
             static_cast<BlockWorldPositionCoord>(glm::floor(position.y)),
             static_cast<BlockWorldPositionCoord>(glm::floor(position.z)) };
}

hvox::BlockWorldPosition hvox::block_world_position(
    ChunkGridPosition  chunk_grid_position,
    BlockChunkPosition block_chunk_position /*= BlockChunkPosition{0.0f}*/
) {
    return {
        static_cast<BlockWorldPositionCoord>(chunk_grid_position.x) * CHUNK_LENGTH
            + block_chunk_position.x,
        static_cast<BlockWorldPositionCoord>(chunk_grid_position.y) * CHUNK_LENGTH
            + block_chunk_position.y,
        static_cast<BlockWorldPositionCoord>(chunk_grid_position.z) * CHUNK_LENGTH
            + block_chunk_position.z,
    };
}

hvox::BlockWorldPosition
hvox::block_world_position(ChunkGridPosition chunk_grid_position, ui32 index) {
