...
hvox::ChunkInstance& hvox::ChunkInstanceManager::get(std::unique_lock<std::shared_mutex>& lock) {
    lock = std::unique_lock(m_mutex);
    return m_instance;
}

const hvox::ChunkInstance& hvox::ChunkInstanceManager::get(std::shared_lock<std::shared_mutex>& lock) {
