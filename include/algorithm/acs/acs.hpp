...
namespace hemlock {
    namespace algorithm {
        ...
        // TODO(Matthew): Don't necessarily want to provide a graph map - scenarios
        // exist where
        //                no sharing can or will occur and thus it may be faster to
        //                deal simply with VertexData.
        // TODO(Matthew): Can we drop pheromone proportionate to some metric (naively
        // degree of
        //                distance reduction on path) so long as no ant has yet
        //                actually reached the target?
        // TODO(Matthew): Allow a variation where the destination is a comparator.
        // Note that here
        //                we will need to allow providing an optional metric for
        //                determining distance, and where not given the algorithm will
        //                have to disable some components.
        // TODO(Matthew): Implement backtracking, it's the only sane option! Track
        // groups by path
        //                length & current vertex.
        template <
            typename VertexData,
            typename NextActionFinder     = NextActionFromGraphFinder<VertexData>,
            typename VertexChoiceStrategy = DefaultChoiceStrategy<VertexData>>
        class BasicACS {
        public:
            BasicACS();
        ...
        protected:
            ...
            size_t m_break_on_path_change;  //< The degree of path change that is
                                            // satisfactory.
            size_t m_break_on_iterations;   //< The number of iterations in which
                                            // satisfactory path
                                            //  change occurs on which to break the
                                            //  path search.
            ...
            f32 m_global_increment;    //< The increment applied in global pheromone
                                       // update.
            f32 m_global_evaporation;  //< The evaporation applied in global pheromone
                                       // update.
        };
    }  // namespace algorithm
}  // namespace hemlock
