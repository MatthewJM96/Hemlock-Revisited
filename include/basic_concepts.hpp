...
namespace hemlock {
    namespace impl {
        ...
        concept BitReference = requires (
            ...
            bool                boolean_stub
                                   ...
                                   };
                                   {
                                       ...
                                       } -> std::same_as<_BitReference&>;
                                   {
                                       bit_reference = boolean_stub
                                       } -> std::same_as<_BitReference&>;
                                   {
                                       bit_reference == bit_reference
                                       } -> std::same_as<bool>;
                                   ...
                                   {
                                       bit_reference == const_bit_reference
                                       } -> std::same_as<bool>;
                                   {
                                       bit_reference < const_bit_reference
                                       } -> std::same_as<bool>;
        ...
        concept LegacyInputIterator
            = LegacyIterator<Iterator> && std::equality_comparable<Iterator>
              && requires (Iterator i) {
        ...
        concept LegacyForwardIterator
            = LegacyInputIterator<Iterator> && std::constructible_from<Iterator>
              ...
              && requires (Iterator i) {
                     {
                         i++
                         } -> std::convertible_to<const Iterator&>;
                     {
                         *i++
        ...
        concept LegacyBidirectionalIterator
            = LegacyForwardIterator<Iterator, IsBitPacked>
              && requires (Iterator i) {
                     {
                         --i
                         } -> std::same_as<Iterator&>;
        ...
        concept LegacyRandomAccessIterator
            = LegacyBidirectionalIterator<Iterator, IsBitPacked>
              && std::totally_ordered<Iterator>
              && requires (
                  Iterator                                                      i,
                  typename std::incrementable_traits<Iterator>::difference_type n
              ) {
                     {
                         i += n
                         } -> std::same_as<Iterator&>;
                     ...
                     {
                         i[n]
                         } -> std::convertible_to<std::iter_reference_t<Iterator>>;
                 };
    }  // namespace impl
    ...
    concept ResizableContainer
        = Container<ContainerType> && requires (ContainerType c, size_t n) {
                                          {
                                              c.resize(n)
                                              } -> std::same_as<void>;
                                      };
    ...
    concept FixedSizeContainer
        = Container<ContainerType> && requires (ContainerType c) {
                                          {
                                              std::tuple_size_v<ContainerType>
                                              } -> std::same_as<size_t>;
                                      };
    ...
    concept ContiguousContainer
        = Container<ContainerType>
          && impl::LegacyRandomAccessIterator<
              typename ContainerType::iterator,
              std::conditional_t<
          ...
          && impl::LegacyRandomAccessIterator<
              ...
              std::conditional_t<
                  ...
                  typename std::false_type>>
          && (
              // TODO(Matthew): What can be used to check here? _Bit_iterator returns
              // newly constructed _Bit_reference on
              //                dereference & _Bit_reference only returns bools by
              //                copy.
              std::same_as<typename ContainerType::value_type, bool>
    ...
    concept ResizableContiguousContainer
        = ContiguousContainer<ContainerType> && ResizableContainer<ContainerType>;
     ...
     */
    template <typename ContainerType>
    concept FixedSizeContiguousContainer
        = ContiguousContainer<ContainerType> && FixedSizeContainer<ContainerType>;
     ...
     */
    template <typename ContainerType>
    concept MapContainer
        = Container<ContainerType>
          && requires (ContainerType c) {
                 typename ContainerType::key_type;
                 typename ContainerType::hasher;
                 typename ContainerType::key_equal;
             } && requires (ContainerType c, typename ContainerType::key_type key, typename ContainerType::mapped_type value, typename ContainerType::iterator it, typename ContainerType::mapped_type const_it) {
                      {
                          c()[key]
                          } -> std::same_as<typename ContainerType::mapped_type&>;
                      {
                          c().at(key)
                          } -> std::same_as<typename ContainerType::mapped_type&>;
                      {
                          c().insert({ key, value })
                              ...
                              std::pair<typename ContainerType::iterator, bool>>;
                      {
                          c().emplace()
                          } -> std::same_as<
                              std::pair<typename ContainerType::iterator, bool>>;
    ...
    concept RPredicate
        = std::regular_invocable<Invocable, Args...>
