...
#  define H_SWITCH_UNION_CASE(KIND_TYPE, KIND) } case KIND_TYPE ::  KIND: {

#  if !defined(H_WRITE_UNION_FIELD_ENCODE)
#    define H_WRITE_UNION_FIELD_ENCODE(FIELD_DATA)  \
node[H_FIELD_NAME_STR FIELD_DATA] = YAML::Node(     \
    val.H_FIELD_NAME FIELD_DATA                     \
);
...
if (                                                \
    !convert<H_FIELD_TYPE FIELD_DATA>::decode(      \
        node[H_FIELD_NAME_STR FIELD_DATA],          \
        result.H_FIELD_NAME FIELD_DATA              \
    )                                               \
) {                                                 \
    return false;                                   \
}
...
namespace NAMESPACE {                                                       \
    enum class NAME##Kind : KIND_TYPE {                                     \
        MAP(H_WRITE_UNION_KIND, EMPTY, __VA_ARGS__)                         \
        SENTINEL                                                            \
    };                                                                      \
    struct NAME {                                                           \
        NAME##Kind kind;                                                    \
