...
void hvox::set_per_block_data(
     ...
     */
    if (start_block_position == hvox::BlockChunkPosition{ 0 }
        && end_block_position == hvox::BlockChunkPosition{ CHUNK_LENGTH - 1 })
    {
    ...
    {
        auto batch_size
            = CHUNK_AREA * (1 + end_block_position.z - start_block_position.z);
    ...
    {
        auto batch_size
            = CHUNK_LENGTH * (1 + end_block_position.y - start_block_position.y);
        for (auto z = start_block_position.z; z <= end_block_position.z; ++z) {
            auto start_idx = block_index({ 0, start_block_position.y, z });
            std::fill_n(&(buffer[start_idx]), batch_size, data);
        }
        /*
        ...
        for (auto z = start_block_position.z; z <= end_block_position.z; ++z) {
            for (auto y = start_block_position.y; y <= end_block_position.y; ++y) {
                auto start_idx = block_index({ start_block_position.x, y, z });
                std::fill_n(&(buffer[start_idx]), batch_size, data);
            }
        }
    }
...
void hvox::set_per_block_data(
     ...
     */
    if (start_block_position == BlockChunkPosition{ 0 }
        && end_block_position == BlockChunkPosition{ CHUNK_LENGTH })
    {
    ...
    } else if (start_block_position.xy() == BlockChunkPosition2D{0} && end_block_position.xy() == BlockChunkPosition2D{CHUNK_LENGTH})
    {
        ...
        for (auto z = start_block_position.z; z < end_block_position.z; ++z) {
            for (auto y = start_block_position.y; y < end_block_position.y; ++y) {
                auto chunk_blocks_start_idx
                    = block_index({ start_block_position.x, y, z });
                auto new_blocks_start_idx
                    = batch_size * (y - start_block_position.y)
                      + batch_size * (end_block_position.y - start_block_position.y)
                            * (z - start_block_position.z);
                std::memcpy(
                    &(buffer[chunk_blocks_start_idx]),
                    &(data[new_blocks_start_idx]),
                    batch_size
