                                ...
                                DataType data )
{
     ...
     */
    if (start_block_position == hvox::BlockChunkPosition{0} && end_block_position == hvox::BlockChunkPosition{CHUNK_SIZE - 1}) {
        std::fill_n(buffer, CHUNK_SIZE * CHUNK_SIZE * CHUNK_SIZE, data);
    /*
     * If we span the XY plane, then we set the whole cuboid as all
     * elements it touches are contiguous in memory.
     */
    } else if (start_block_position.xy() == hvox::BlockChunkPosition2D{0} && end_block_position.xy() == hvox::BlockChunkPosition2D{CHUNK_SIZE - 1}) {
                              ...
                              DataType* data )
{
    ...
    if (start_block_position == BlockChunkPosition{0} && end_block_position == BlockChunkPosition{CHUNK_SIZE}) {
        std::memcpy(buffer, data, CHUNK_SIZE * CHUNK_SIZE * CHUNK_SIZE);
    /*
