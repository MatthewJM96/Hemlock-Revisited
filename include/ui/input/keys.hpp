...
#define PUTASYM(X, Y) H_##X = SDL_SCANCODE_##Y,
            PUT(0)
            PUT(1) PUT(2) PUT(3) PUT(4) PUT(5) PUT(6) PUT(7) PUT(8) PUT(9) PUT(A
            ) PUT(B) PUT(C) PUT(D) PUT(E) PUT(F) PUT(G) PUT(H) PUT(I) PUT(J) PUT(K
            ) PUT(L) PUT(M) PUT(N) PUT(O) PUT(P) PUT(Q) PUT(R) PUT(S) PUT(T) PUT(U
            ) PUT(V) PUT(W) PUT(X) PUT(Y) PUT(Z) PUT(F1) PUT(F2) PUT(F3) PUT(F4
            ) PUT(F5) PUT(F6) PUT(F7) PUT(F8) PUT(F9) PUT(F10) PUT(F11) PUT(F12
            ) PUT(F13) PUT(F14) PUT(F15) PUT(F16
            ) PUT(F17) PUT(F18) PUT(F19) PUT(F20
            ) PUT(F21) PUT(F22) PUT(F23) PUT(F24
            ) PUT(AC_BACK) PUT(AC_BOOKMARKS) PUT(AC_FORWARD
            ...
            ) PUT(KP_OCTAL) PUT(KP_PERCENT) PUT(KP_PERIOD
                ...
                ) PUT(SPACE) PUT(STOP) PUT(SYSREQ) PUT(TAB) PUT(THOUSANDSSEPARATOR
                ) PUT(UNDO) PUT(UNKNOWN) PUT(UP) PUT(VOLUMEDOWN) PUT(VOLUMEUP
                ) PUT(WWW) PUT(INTERNATIONAL1) PUT(INTERNATIONAL2) PUT(INTERNATIONAL3)
                    PUT(INTERNATIONAL4)
                        PUT(INTERNATIONAL5)
                            PUT(INTERNATIONAL6)
                                PUT(INTERNATIONAL7)
                                    PUT(INTERNATIONAL8)
                                        PUT(INTERNATIONAL9)
                                            PUT(LANG1) PUT(LANG2) PUT(LANG3)
                                                PUT(LANG4) PUT(LANG5) PUT(LANG6)
                                                    PUT(LANG7) PUT(LANG8) PUT(LANG9)
                                                        PUT(NONUSBACKSLASH)
                                                            PUT(NONUSHASH)
#undef PUTASYM
#undef PUT
...
#define PUTASYM(X, Y) H_##X = SDLK_##Y,
            ...
            PUT(1) PUT(2) PUT(3) PUT(4) PUT(5) PUT(6) PUT(7) PUT(8) PUT(9
                ...
                PUTASYM(F, f) PUTASYM(G, g) PUTASYM(H, h) PUTASYM(I, i) PUTASYM(
                    J, j
                ) PUTASYM(K, k) PUTASYM(L, l) PUTASYM(M, m) PUTASYM(N, n)
                    PUTASYM(O, o) PUTASYM(P, p) PUTASYM(Q, q) PUTASYM(R, r) PUTASYM(
                        S, s
                    ) PUTASYM(T, t) PUTASYM(U, u) PUTASYM(V, v) PUTASYM(W, w)
                        PUTASYM(X, x) PUTASYM(Y, y) PUTASYM(Z, z) PUT(F1) PUT(F2
                        ) PUT(F3) PUT(F4) PUT(F5) PUT(F6) PUT(F7) PUT(F8) PUT(F9
                        ) PUT(F10) PUT(F11) PUT(F12) PUT(F13) PUT(F14) PUT(F15
                        ) PUT(F16) PUT(F17) PUT(F18) PUT(F19) PUT(F20) PUT(F21
                        ) PUT(F22) PUT(F23) PUT(F24) PUT(AC_BACK) PUT(AC_BOOKMARKS
                        ) PUT(AC_FORWARD) PUT(AC_HOME) PUT(AC_REFRESH) PUT(AC_SEARCH
                        ) PUT(AC_STOP) PUT(AGAIN) PUT(ALTERASE
                        ) PUTASYM(APOSTROPHE, QUOTE) PUT(APPLICATION) PUT(AUDIOMUTE
                        ) PUT(AUDIONEXT) PUT(AUDIOPLAY) PUT(AUDIOPREV) PUT(AUDIOSTOP
                        ) PUT(BACKSLASH) PUT(BACKSPACE) PUT(BRIGHTNESSDOWN
                        ) PUT(BRIGHTNESSUP) PUT(CALCULATOR) PUT(CANCEL) PUT(CAPSLOCK
                        ) PUT(CLEAR) PUT(CLEARAGAIN) PUT(COMMA) PUT(COMPUTER
                        ...
                        ) PUT(KBDILLUMDOWN) PUT(KBDILLUMTOGGLE) PUT(KBDILLUMUP
                        ) PUT(KP_0) PUT(KP_00) PUT(KP_000) PUT(KP_1) PUT(KP_2
                        ) PUT(KP_3) PUT(KP_4
                        ) PUT(KP_5) PUT(KP_6
                        ) PUT(KP_7) PUT(KP_8
                        ) PUT(KP_9) PUT(KP_A
                        ...
                        ) PUT(KP_PLUS) PUT(KP_PLUSMINUS) PUT(KP_POWER
                        ) PUT(KP_RIGHTBRACE) PUT(KP_RIGHTPAREN
                        ...
                        ) PUT(MEDIASELECT) PUT(MENU) PUT(MINUS) PUT(MODE) PUT(MUTE
                        ) PUT(NUMLOCKCLEAR) PUT(OPER
                        ) PUT(OUT) PUT(PAGEDOWN) PUT(PAGEUP) PUT(PASTE
                        ) PUT(PAUSE) PUT(PERIOD) PUT(POWER) PUT(PRINTSCREEN) PUT(PRIOR
                        ) PUT(RALT) PUT(RCTRL) PUT(RETURN) PUT(RETURN2
                        ) PUTASYM(RCMD, RGUI) PUT(RIGHT
                        ...
                        ) PUTASYM(INTERNATIONAL1, UNKNOWN)
                            PUTASYM(INTERNATIONAL2, UNKNOWN) PUTASYM(
                                INTERNATIONAL3,
                                UNKNOWN
                            ) PUTASYM(INTERNATIONAL4, UNKNOWN)
                                PUTASYM(INTERNATIONAL5, UNKNOWN)
                                    PUTASYM(INTERNATIONAL6, UNKNOWN)
                                        PUTASYM(INTERNATIONAL7, UNKNOWN)
                                            PUTASYM(INTERNATIONAL8, UNKNOWN) PUTASYM(
                                                INTERNATIONAL9, UNKNOWN
                                            ) PUTASYM(LANG1, UNKNOWN)
                                                PUTASYM(LANG2, UNKNOWN) PUTASYM(
                                                    LANG3, UNKNOWN
