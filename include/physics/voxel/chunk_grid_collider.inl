...
bool hphys::ChunkGridCollider::determine_candidate_colliding_voxels(
    ...
    //                  ray casting to be used to choose which voxels to include
    ...
    auto min_local_coord
        = hvox::voxel_world_position(f32v3(min_aabb.x(), min_aabb.y(), min_aabb.z()));
    auto max_local_coord
        = hvox::voxel_world_position(f32v3(max_aabb.x(), max_aabb.y(), max_aabb.z()));
    ...
    auto min_world_voxel_coord = hvox::VoxelWorldPosition{
        static_cast<hvox::VoxelWorldPositionCoord>(min_world_entity_coord.x >> 32),
        static_cast<hvox::VoxelWorldPositionCoord>(min_world_entity_coord.y >> 32),
        static_cast<hvox::VoxelWorldPositionCoord>(min_world_entity_coord.z >> 32)
    };
    auto max_world_voxel_coord = hvox::VoxelWorldPosition{
        static_cast<hvox::VoxelWorldPositionCoord>(max_world_entity_coord.x >> 32),
        static_cast<hvox::VoxelWorldPositionCoord>(max_world_entity_coord.y >> 32),
        static_cast<hvox::VoxelWorldPositionCoord>(max_world_entity_coord.z >> 32)
    };

    btVector3 half_range = btVector3{ (static_cast<btScalar>(max_world_voxel_coord.x)
                                       - static_cast<btScalar>(min_world_voxel_coord.x))
                                          / 2.0f,
                                      (static_cast<btScalar>(max_world_voxel_coord.y)
                                       - static_cast<btScalar>(min_world_voxel_coord.y))
                                          / 2.0f,
                                      (static_cast<btScalar>(max_world_voxel_coord.z)
                                       - static_cast<btScalar>(min_world_voxel_coord.z))
                                          / 2.0f };

    auto new_chunk_coord = hvox::chunk_grid_position(min_world_voxel_coord);
    ...
    const hvox::Voxel*                  voxels;

    for (auto x = min_world_voxel_coord.x; x < max_world_voxel_coord.x; ++x) {
        for (auto y = min_world_voxel_coord.y; y < max_world_voxel_coord.y; ++y) {
            for (auto z = min_world_voxel_coord.z; z < max_world_voxel_coord.z; ++z) {
                ...
                if (chunk == nullptr || (old_chunk_coord != new_chunk_coord)) {
                    ...
                    // Considering voxel in new chunk, get handle on that new
                    // chunk and check it at least exists.
                    chunk = chunk_grid->chunk(new_chunk_coord);
                    if (chunk == nullptr) continue;

                    // Chunk exists, get lock on voxels.
                    voxels = chunk->voxels.get(lock);

                    old_chunk_coord = new_chunk_coord;
                }

                auto voxel_idx
                    = hvox::voxel_index(hvox::voxel_chunk_position({ x, y, z }));
                auto voxel = voxels[voxel_idx];

                btTransform       transform = btTransform::getIdentity();
                btCollisionShape* shape     = shape_evaluator(voxel, transform);
                if (shape) {
                    ...
                    colliders->addChildShape(transform, shape);
