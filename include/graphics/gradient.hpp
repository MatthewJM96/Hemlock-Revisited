...
namespace hemlock {
    namespace graphics {
         ...
         *
         * @tparam Precision The type to use for the ratio for interpolating - this must be a
         * floating-point type.
         *
        ...
        colour3 lerp(colour3 c1, colour3 c2, Precision ratio) {
            return {
                static_cast<ui8>(static_cast<Precision>(c1.r) * (1.0 - ratio) + static_cast<Precision>(c2.r) * ratio),
                static_cast<ui8>(static_cast<Precision>(c1.g) * (1.0 - ratio) + static_cast<Precision>(c2.g) * ratio),
                static_cast<ui8>(static_cast<Precision>(c1.b) * (1.0 - ratio) + static_cast<Precision>(c2.b) * ratio)
            };
        }
        ...
        colour4 lerp(colour4 c1, colour4 c2, Precision ratio) {
            return {
                ...
                static_cast<ui8>(static_cast<Precision>(c1.g) * (1.0 - ratio) + static_cast<Precision>(c2.g) * ratio),
                static_cast<ui8>(static_cast<Precision>(c1.b) * (1.0 - ratio) + static_cast<Precision>(c2.b) * ratio),
                static_cast<ui8>(static_cast<Precision>(c1.a) * (1.0 - ratio) + static_cast<Precision>(c2.a) * ratio)
            };
        }
    }
}
